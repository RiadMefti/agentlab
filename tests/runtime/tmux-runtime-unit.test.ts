  ...
  public run(executable: string, args: readonly string[]): Promise<RunResult> {
    ...
    if (subcommand === "list-sessions") {
      ...
      return Promise.resolve({
        stdout: this.listedSessions ?? `${name}|1787313600|1|0\nignored-session|0|0|0\n`,
        stderr: ""
      });
    }
...
describe("TmuxSessionRuntime", () => {
  ...
  it("does not invent an epoch timestamp when tmux omits the creation field", async () => {
    const name = buildCaptainSessionName(TEST_CONVERSATION_ID, "codex");
    const runtime = new TmuxSessionRuntime(new RecordingRunner(false, `${name}||0|0\n`));

    await expect(runtime.list(TEST_CONVERSATION_ID)).resolves.toMatchObject([
      { name, startedAt: null }
    ]);
  });

  it("keeps a managed session visible when tmux returns an out-of-range timestamp", async () => {
    const name = buildCaptainSessionName(TEST_CONVERSATION_ID, "codex");
    const runtime = new TmuxSessionRuntime(
      new RecordingRunner(false, `${name}|999999999999999999999|0|0\n`)
    );
